...
ARENA FACTS: 2.4m square, checkerboard floor (200mm tiles), striped walls with orange top band, five colored blocks (red, blue, green, yellow, purple) at fixed unknown-to-you positions. The rover has a FIXED forward camera (no pan — looking elsewhere means turning the rover; never write camera-pan steps), 3 forward ultrasonics, and a memory map that grows as it drives.
Break the mission into an EXHAUSTIVE ordered list of 3-8 concrete executable steps. The executor offers exactly these motions: scan, goto(landmark), follow_waypoints (plotted arcs/routes), move, turn — write steps that map cleanly onto them (e.g. 'plot and drive an arc around the red block to its far face'). Each step must be something a rover can do by driving/turning/looking, with an observable success condition. Cover: locating any named target (rotating/scanning until seen), approaching it, positioning (seeing a far face requires driving AROUND the object), verifying, and answering.
    ...
    try:
        ...
        if name == "move":
            ...
            clean = {"name": "mark", "args": {"label": str(args.get("label", "poi"))[:20],
                     "x": _f(args.get("x"), -1150, 1150, 0), "y": _f(args.get("y"), -1150, 1150, 0)}}
        elif name == "stop":
