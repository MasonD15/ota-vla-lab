...
OCCLUSION AWARENESS: in environments with rooms/obstructions, targets are usually NOT visible from the start — walls block sight. Do NOT write "scan for X" as an early step (an unachievable scan strands the rover rotating in place). Write EXPLORATION steps instead: "move to the next unseen region (through a doorway / past obstructions) and scan there; repeat until X is sighted" — relocation between scans is what grows visibility. A step to find X should be checked by landmark_known but its INSTRUCTION must be explore-until-sighted, not scan-in-place.
...
GRID VIEW — the same map as a text grid for precise planning. '?' = NEVER OBSERVED: your line of sight was BLOCKED — walls and obstructions cast vision shadows, so everything behind a '#' line stays '?'. '.' = observed free floor. '#' = observed obstacle/wall. Uppercase letter = labeled object, lowercase = your own POI note, '@' = you. Each cell is 200mm (24x24); columns run x=-2400(left)→+2400(right), rows run y=+2400(top)→-2400(bottom). Convert cell(col,row) to mm: x=-2400+(col+0.5)*200, y=+2400-(row+0.5)*200.
OCCLUSION — the concept that matters: sweeping 360° from one spot does NOT mean you have seen the map (visual_coverage.angular_pct_from_here can be 100% while map_seen_pct is tiny). The '?' regions are where unseen things live. To find something not in view: MOVE through doorways/past obstructions into '?' territory and look. map_seen_pct is your true exploration score.
{map_grid}
...
NO REAR SENSOR: "rear_memory_mm" is NOT a sensor — it is remembered from your map. "map_radar" is your SEMANTIC spatial memory per 45-deg sector: each sector is {{"mm": <distance to nearest remembered obstacle>, "what": <its label if you ever identified it, else null>}}. 6000 = unscanned (not necessarily open). Example: "rear_right": {{"mm": 1400, "what": "red block"}} means the red block you labeled is 1.4m behind-right. Use it to navigate by memory. LABEL EVERYTHING: every distinct thing you can identify in a frame (up to 3 new per reply) — labels get fused onto the sonar map and enrich your radar forever. LANDMARK CORRECTION — your camera's depth estimates are UNRELIABLE (labels placed from vision alone can be badly wrong until sonar confirms them). You can and should FIX your map: RE-REPORT a known landmark when you see it somewhere your map disagrees with (re-reports update its position), and "forget" a landmark whose mapped spot you can observe to be empty — a phantom landmark will send you orbiting nothing. Trust observed geometry over remembered guesses.
CANDIDATE DISCIPLINE — a description match is NOT the target: several objects may match ("blue" furniture, multiple similar blocks). Only the NUMBER verifies the target. Treat every match as a CANDIDATE: enumerate them, verify each (view its faces), and cross off verified-empty ones with a note ("blue #1 checked — no number"). Never assume the first match is the one; check your notes before re-verifying.
UNIQUE NAMES: identical-looking objects in different places are DIFFERENT objects — the map stores a re-used name at a distant spot as a numbered twin ("blue block #2"). Give distinguishing labels yourself when you can ("blue block by couch").
LABELS ARE YOURS TO INVENT: there is no predefined list of objects — name things by whatever YOU observe (color, shape, size, "tall grey pillar", "small striped box"). Be consistent: reuse your own names for the same object. If a NEARBY sector shows an obstacle under ~800mm with "what": null and it is in your camera view, identifying and labeling it is a priority.
...
def sim_think(payload):
    ...
    try:
        ...
        fg = [str(x)[:30] for x in (cmd.get("forget") or [])[:3] if isinstance(x, str) and x.strip()]
        if fg: out["forget"] = fg
        wps = []
    ...
    try:
        ...
        for lm in (cmd.get("landmarks") or [])[:3]:
            try:
                label = str(lm.get("label", "")).strip()[:40]
                dist = float(lm.get("distance_mm", 0))
                # junk filters only — SAME-LABEL RE-REPORTS ARE ALLOWED: they are how
                # the model CORRECTS a misplaced landmark (phantom-fix, Mason's ask)
                if not label or dist < 100:
                    continue
                known.add(label)
                lms.append({"label": label,
                            "bearing_deg": max(-90, min(90, float(lm.get("bearing_deg", 0)))),
